...
Creates following JMS Topic items:
    ...
    ProductEventDistributedTopicModule
    ProductEventDistributedTopicSubDeployment
    ProductEventDistributedTopicDistributedQueue
...
if server1 is None:
    ...
    jmsModule = create('ProductEventModule','JMSSystemResource')
    jmsModule.addTarget(server1)

    " Target the sub-deployment to the JMS server"
    subDeployment = jmsModule.createSubDeployment('ProductEventSubDeployment')
    ...
    jmsQueueError = jmsResource.createUniformDistributedQueue('ProductEventDistributedQueueError')
    jmsQueueError.setJNDIName('jms.ProductEventDistributedQueueError')
    jmsQueueError.setSubDeploymentName('ProductEventSubDeployment')
    print "MBean type UniformDistributedQueue with name ProductEventDistributedQueueError has been created successfully"
    
    
    jmsQueue = jmsResource.createUniformDistributedTopic('ProductEventDistributedTopic')
    jmsQueue.setJNDIName('jms.ProductEventDistributedTopic')
    jmsQueue.setSubDeploymentName('ProductEventSubDeployment')
    ...
    print "MBean type UniformDistributedTopic with name ProductEventDistributedTopic has been created successfully"
